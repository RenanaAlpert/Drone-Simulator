...
public class AutoAlgo1 {
	int points_to_check_distanse_from = 5;
	int spin_by = 0;
	...
	public void ai(int deltaTime) {
		...
		if (SimulationWindow.return_home) {
			if (!try_to_escape) {
				ArrayList<Point> close_points = get_x_LastPoint(points_to_check_distanse_from);
		...
		if (!is_risky) {
			...
			if (lidar1.current_distance <= (WorldParams.safety_distance + WorldParams.extra_range + WorldParams.lidarLimit)) {//was  max_risky_distance / 3
				is_risky = true;
			}
			Lidar lidar2 = drone.lidars.get(2);
			if (lidar2.current_distance <= (WorldParams.safety_distance + WorldParams.extra_range + WorldParams.lidarLimit)) {//was  max_risky_distance / 3
				is_risky = true;
			}
